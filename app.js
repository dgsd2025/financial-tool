...
const RS_YUNDI = {
  ...
  owners: {},
  ownerAcct: '', ownerMemo: '',
  /* 出资人名单：注资类流水摘要常是「转账」，没有业务含义，只能靠户名认 */
  investors: ['李堪珍'],
  rules: [
    { kw: '证书工本费|网银证书|工本费|证书服务费', dir: 'out', acct: '560303', memo: '银行证书费' },
    { kw: '手续费|账户管理费|短信费', dir: 'out', acct: '560303', memo: '银行手续费' },
    { kw: '注资|投资款|出资|实收资本', dir: 'in', acct: '4001', memo: '收到出资',
      warn: '股东/合伙人资金动作，须触发独立报告线并留存出资手续' },
...
function runRules() {
  ...
  body.forEach((r, i) => {
    ...
    let hitField = '';
    // 对方户名在出资人名单里 + 收款方向 → 判定为收到出资
    if (RS && RS.investors && dir === 'in') {
      const inv = RS.investors.find(n => opp && (opp === n || opp.includes(n)));
      if (inv) {
        rec.rule = { id: 'investor', kw: '出资人名单', memo: '收到出资' };
        rec.hitField = '出资人名单'; rec.proj = null;
        rec.acct = '4001'; rec.vmemo = '收到 ' + inv + ' 出资';
        rec.tax = 0; rec.red = 0;
        rec.warn = '股东/合伙人资金动作，须触发独立报告线并留存出资手续';
        ok.push(rec); return;
      }
    }
